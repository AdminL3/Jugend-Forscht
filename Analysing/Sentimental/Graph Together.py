...
for o, option in enumerate(options):
    ...
    for i, topic in enumerate(topics):
        ...
        plt.plot(Dataframe.index,
                Dataframe[option], 'o', markersize=2, color=colors[i])
    ...
    legend1 = [f"{name} for {topic}" for topic in topics]
    ...
    plt.savefig(f"Analysing\Sentimental\output\{option}\Together.png")
    plt.close()
